...
export const synchronizeEvents = async (discordClient: Client, openseaClient: OpenSeaClient) => {
    ...
    const promises = notificationSubscriptions.map(async (notificationSubscription) => {
        ...
        const { events } = await openseaClient.getCollectionEvents(slug, type, notificationSubscription.lastSyncAt ? lastSync : undefined);
        ...
        (!notificationSubscription.lastSyncAt ? events : [events[0]]).forEach((event) => {
            if (!event) return;
            ...
            if (type === 'created') {
                const embed = new MessageEmbed()
                    ...
                    .addField('Amount', `${parseInt(eventData.starting_price) / 1E18 || '0'} ETH`)
